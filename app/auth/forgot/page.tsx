...
export default function ForgotPassword() {
  ...
  const handleResetPassword = async (
    ...
    if (!error) {
      setNotifyText({
        isSuccess: true,
        alertText: "Please check your email for link to reset your password.",
      });
    } else {
      setNotifyText({
        isSuccess: false,
        alertText: "Error sending reset password email.",
      });
    }
    setEmail("");
  };
        ...
        <form onSubmit={handleResetPassword} className="space-y-4">
          <CardHeader>
            <div className="flex items-center justify-center mb-10">
              <Image src={ic_password} alt="lock icon" priority />
            </div>
            <CardTitle className="text-[32px] font-bold text-center mb-3">
